...
public class PokerTableController {
	...
	private void handlePlay() {
		...
		if (mainApp.getiGameType() == null) {
			msgLbl.setText("Choose a Game!");
			FadeTransition ft = new FadeTransition(Duration.millis(3000), msgLbl);
		     ft.setFromValue(1.0);
		     ft.setToValue(0.0);
		     ft.play();
		}
		else if (mainApp.GetSeatedPlayers().size() == 0) {
			msgLbl.setText("No Players are Seated");
			FadeTransition ft = new FadeTransition(Duration.millis(3000), msgLbl);
		     ft.setFromValue(1.0);
		     ft.setToValue(0.0);
		     ft.play();
		...
		eGame gameRule = mainApp.getiGameType();
		Rule rle = new Rule(gameRule);
	...
	private void handleDraw() {
		...
		if (mainApp.GetSeatedPlayers().size() == 0) {
			msgLbl.setText("No Players are Seated");
			FadeTransition ft = new FadeTransition(Duration.millis(3000), msgLbl);
		     ft.setFromValue(1.0);
		     ft.setToValue(0.0);
		...
		else if (mainApp.getiGameType() == null) {
			msgLbl.setText("Choose a Game!");
			FadeTransition ft = new FadeTransition(Duration.millis(3000), msgLbl);
		     ft.setFromValue(1.0);
		     ft.setToValue(0.0);
		...
		if (act.isbEvaluateHand()) {
			...
			/*
			 * could not figure out how to identify location of cards once they are nodes, so tried to
			 * provide a visual representation of winning cards this way. ImageView used this way
			 * does not work, however, though it should, as you used it in the same way in other 
			 * parts of the code.
			 */
			for (Card c : WinningHand.getCardsInHand()) {
				String cardStr = "res/img/" + c.getCardImg();
				ImageView imgvCard = new ImageView(new Image(getClass().getResourceAsStream(cardStr), 75, 75, true, true));
				winCards.getChildren().add(imgvCard);
			}
	...
	private void showWinner(Player p) {
		switch(p.getiPlayerPosition()) {
		case 1: winLbl.setText(txtP1Name.getText() + " is the Winner!!");
				break;
		case 2: winLbl.setText(txtP2Name.getText() + " is the Winner!!");
				break;
		case 3: winLbl.setText(txtP3Name.getText() + " is the Winner!!");
				break;
		case 4: winLbl.setText(txtP4Name.getText() + " is the Winner!!");
				break;
		}
		FadeTransition ft = new FadeTransition(Duration.millis(1000), winLbl);
	     ft.setFromValue(1.0);
	     ft.setToValue(0.0);
	     ft.setCycleCount(4);
